function Euler_Motion_Eq6_fruitfly_4itemTorq2_1_phaseshift
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% k_hinge=0.97;         % 较好的paper级结果
% k_hinge=1.11;         %  % 计算的扭转角幅值≈实测扭转角幅值
% k_hinge=0.865;       % 
% k_hinge=1.03;         % 
% k_hinge=0.6*1.0;    % K_hinge=1.411uN.mm; f_n =380.5327;lambda =2.0166;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pitchhinge_coef=pitch_hinge_rigid_coefficients();
%%%%%%%%%%%%%%%%%%%%%%%%%
% 针对 lambda=linspace(1,3,9);
% pitchhinge_coef =[0.3095,0.4836,0.6964,0.9479,1.2381,1.5670,1.9345,2.3408,2.7857];
% k_hinge=pitchhinge_coef(1,1);      % delta =11.8560;
% k_hinge=pitchhinge_coef(1,2);          % delta =8.2685;
% k_hinge=pitchhinge_coef(1,3);      % delta =8.4591;
% k_hinge=pitchhinge_coef(1,4)      %   delta =9.6136;
% k_hinge=pitchhinge_coef(1,5);      % delta =4.5086;
% k_hinge=pitchhinge_coef(1,6);      % delta =2.4624;
% k_hinge=pitchhinge_coef(1,7);          % delta =0;
% k_hinge=pitchhinge_coef(1,8);     %  delta =-2.9686;
% k_hinge=pitchhinge_coef(1,9);    % delta =-3.1879;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% k_hinge=pitchhinge_coef(1,1);      % delta =8.4591;
% k_hinge=pitchhinge_coef(1,2);      % delta =7.0027;
k_hinge=pitchhinge_coef(1,3);      % delta =7.2051;
% k_hinge=pitchhinge_coef(1,4)      %   delta =4.5086;
% k_hinge=pitchhinge_coef(1,5);      % delta =3.5464;
% k_hinge=pitchhinge_coef(1,6);      % delta =2.5429;
% k_hinge=pitchhinge_coef(1,7);          % delta =0;
% k_hinge=pitchhinge_coef(1,8);     %  delta =0;
% k_hinge=pitchhinge_coef(1,9);    % delta =-2.9378;
% k_hinge=pitchhinge_coef(1,10);      % delta =-3.1879;
% k_hinge=pitchhinge_coef(1,11);  % delta =-2.9656;  
% k_hinge=pitchhinge_coef(1,12);  % delta =-2.9414;  % delta =-2.9479;
% k_hinge=pitchhinge_coef(1,13);  % delta =-5.9187;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
n_period=2;
t_range=[0.0052824335,0.0052824335+n_period*T];
...
% psi_max=[76.3123,72.6324,69.0907,65.6204,62.2658,59.0059,56.0679,53.4473,51.2758,49.4162,47.8372,46.3858,45.0164];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% xlswrite('PassiveRot_angle_Alpha_sim.xlsx',A,'sheet1','A1:D12000');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 绘制运动学输出
...
% psi_sim=2.0*x(:,1);
% psi_sim=2.515*x(:,1);
stdev_psi_sim_to_exp=std((psi_exp-psi_sim),0,1)   
% stdev_psi_sim_to_exp =[0.2233,0.1915,0.1619,0.1417,0.1330,0.1346,0.1473,0.1665,0.1823,0.2039,0.2336,0.2601,0.2808];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% (B) 实测翅拍运动角输入，模拟被动扭转角和角速度输出
B=[t,phi_exp,psi_exp,psi_sim,dpsi_sim];
xlswrite('wingbeat_angle_exp_pitch_angle_sim.xlsx',B,'sheet1','A1:E12000');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% size(t) % (245*1)
f=1/T;
[phi_globalmin,index_globalmin]=min(phi_exp);
phi_globalmin =phi_globalmin*180/pi
t_phi_global_min=t(index_globalmin)
t_phi_globalmin=t(index_globalmin)/T
% t_interval_half=(t(length(t),1)-t(1,1))/2;
if t_phi_global_min<t(fix(length(t)/2),1)  % 二分搜索
    [phimin1,index1]=min(phi_exp(index_globalmin+1:length(t),1));
    phi_min1 =phimin1*180/pi
    % t_phi_min1=t(index1+index_globalmin+1)
    t_phimin1=t(index1+index_globalmin+1)/T
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % 寻找拍打角为最大值对应的时刻
    [phi_max,in_dex]=max(phi_exp(index_globalmin:index1+index_globalmin+1,1));
    phimax =phi_max*180/pi
    t_phi_max=t(in_dex+index_globalmin)
    t_phimax=t(in_dex+index_globalmin)/T
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % 寻找扭转角为零值点对应的时刻
    [psi_0,index_psi0]=min(abs(psi_exp(index_globalmin+10:index1+index_globalmin+1-10,1)));
    psi0=psi_0*180/pi
    t_psi_0=t(index_psi0+index_globalmin+10)
    t_psi0=t(index_psi0+index_globalmin+10)/T
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % if t_phi_max>t_psi_0
    %    delta=2*pi*f*(t_phi_max-t_psi_0)*180/pi  % advanced phase
    % else
    %  delta=2*pi*f*(t_psi_0-t_phi_max)*180/pi   % delayed phase
    % end
    %%%%%%%%%%%%%%%%%%%%
    delta=2*pi*f*(t_psi_0-t_phi_max)*180/pi   % advanced phase为负值;% delayed phase为正;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
else
    [phimin1,index1]=min(phi_exp(1:index_globalmin-1,1));
    phi_min1 =phimin1*180/pi
    % t_phi_min1=t(index1)
    t_phimin1=t(index1)/T
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % 寻找拍打角为最大值对应的时刻
    [phi_max,in_dex]=max(phi_exp(index1:index_globalmin,1));
    phimax =phi_max*180/pi
    t_phi_max=t(in_dex+index1)
    t_phimax=t(in_dex+index1)/T
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     % 寻找扭转角为零值点对应的时刻
     [psi_0,index_psi0]=min(abs(psi_exp(index1+10:index_globalmin-10,1)));
     psi0=psi_0*180/pi
     t_psi_0=t(index_psi0+index1+10)
    t_psi0=t(index_psi0+index1-10)/T
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % if t_phi_max>t_psi_0
    %    delta=2*pi*f*(t_phi_max-t_psi_0)*180/pi  % advanced phase
    % else
